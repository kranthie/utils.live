...
describe("dockerfileGenerator", () => {
  ...
  describe("execute", () => {
    ...
    it("should generate Python Dockerfile", async () => {
      const result = await executeTool(dockerfileGenerator, { runtime: "python", version: "3.12" });
      expect(result.success).toBe(true);
      if (result.success) {
        const output = (result.data as Record<string, unknown>).output as string;
    ...
    it("should generate single-stage when multiStage is false", async () => {
      const result = await executeTool(dockerfileGenerator, { multiStage: false });
      expect(result.success).toBe(true);
      if (result.success) {
        const output = (result.data as Record<string, unknown>).output as string;
    ...
    it("should generate generic Dockerfile for unsupported runtimes", async () => {
      const result = await executeTool(dockerfileGenerator, { runtime: "ruby" });
      expect(result.success).toBe(true);
      if (result.success) {
        const output = (result.data as Record<string, unknown>).output as string;
