...
import { hjsonToJson } from "../../../src/tools/data/hjson-to-json";
import { executeTool } from "../../../src/core/executor";
import { ToolTier } from "../../../src/types";

describe("hjsonToJson", () => {
  describe("metadata", () => {
    it("should have correct id", () => {
      expect(hjsonToJson.meta.id).toBe("data/hjson-to-json");
    });

    it("should have correct name", () => {
      expect(hjsonToJson.meta.name).toBe("HJSON to JSON");
    });

    it("should be in data category", () => {
      expect(hjsonToJson.meta.category).toBe("data");
    });

    it("should be CLIENT tier", () => {
      expect(hjsonToJson.meta.tier).toBe(ToolTier.CLIENT);
    });

    it("should have relevant keywords", () => {
      expect(hjsonToJson.meta.keywords).toContain("hjson");
      expect(hjsonToJson.meta.keywords).toContain("json");
      expect(hjsonToJson.meta.keywords).toContain("convert");
    });
  });
  ...
  describe("execute", () => {
    it("should convert valid standard JSON", async () => {
      ...
      const result = await executeTool(hjsonToJson, { input });
  ...
  "name": "test"
    ...
    it("should remove hash comments", async () => {
      const input = `{
...
'''
    ...
    it("should handle complex HJSON with multiple features", async () => {
      const input = `{
  ...
  api: {
    ...
    it("should respect custom indent option", async () => {
      const input = '{"name": "test"}';
      const result = await executeTool(hjsonToJson, { input }, { indent: 4 });
    ...
    it("should handle indent of 0 (minified)", async () => {
      const input = '{"name": "test"}';
      const result = await executeTool(hjsonToJson, { input }, { indent: 0 });
  ...
  value: null
    ...
    it("should handle nested objects with unquoted keys", async () => {
      const input = `{
  outer: {
    inner: {
      value: 123
      ...
      if (result.success) {
        ...
        expect(inner.value).toBe(123);
      }
    });

    it("should include execution metadata", async () => {
      const result = await executeTool(hjsonToJson, {
        input: '{"test": true}',
      });
  ...
  describe("options schema", () => {
    it("should have indent option defined", () => {
      expect(hjsonToJson.optionsSchema).toBeDefined();
    });
  ...
  describe("execute function directly", () => {
    it("should convert HJSON when called directly", () => {
      const result = hjsonToJson.execute({ input: '{"name": "test"}' }) as {
        output: string;
      };
    ...
    it("should use default indent when options is undefined", () => {
      const result = hjsonToJson.execute(
        { input: '{"name": "test"}' },
        undefined
